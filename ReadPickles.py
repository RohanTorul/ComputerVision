...
if data:
    ...
    vision_index = 0
    if vision_output is not None or len(vision_output) > 0:
        while True:
            frame, alt, pos = vision_output[vision_index]
            cv2.imshow('valid_frame', frame)
            cv2.setMouseCallback('valid_frameframe', mouse_callback, (alt, pos))
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break
            if cv2.waitKey(1) & 0xFF == 31:# space
                vision_index += (vision_index + 1) % len(vision_output)
                print("now showing frame ", vision_index)
    else: print("No valid frames")

    invalid_index = 0
    if potentially_invalid_frames is not None or len(potentially_invalid_frames) > 0:
        while True:
            frame, alt, pos = potentially_invalid_frames[invalid_index]
            cv2.imshow('invalid_frame', frame)
            cv2.setMouseCallback('invalid_frameframe', mouse_callback, (alt, pos))
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break
            if cv2.waitKey(1) & 0xFF == 31:# space
